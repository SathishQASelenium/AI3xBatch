...
# Windows: when stdout is a pipe or a redirect to a file, Python falls back to
# the ANSI codepage (cp1252), which cannot encode the emoji in our headers or
# any non-ASCII text coming back from Jira. That kills the run with
# UnicodeEncodeError before a single agent speaks. Force UTF-8 on the streams.
...
# OpenRouter has no such per-request squeeze, so when it takes over it gets room.
# (The real ceiling is whatever context window the model you route to exposes.)
...
class FallbackLLM(BaseLLM):
    ...
    OpenRouter is itself a router in front of many model providers, so it adds
    a second failure mode worth knowing about: the upstream model can be down
    or rate-limited even though OpenRouter answers with 200. That surfaces
    here as an empty or truncated response, which _is_empty() already treats
    as a failure.
    ...
    def _is_empty(response):
        ...
        Providers occasionally return empty content on long generations - and
        with OpenRouter you also inherit whatever the upstream model does. That
...
def make_openrouter_llm(max_tokens=OPENROUTER_MAX_TOKENS):
    """OpenRouter is one key in front of many model providers.

    OPENROUTER_MODEL is a full OpenRouter slug ("<vendor>/<model>", e.g.
    "deepseek/deepseek-chat" or "meta-llama/llama-3.3-70b-instruct"), so the
    litellm route prefix goes in FRONT of it and the slug keeps its own slash:
        openrouter/deepseek/deepseek-chat
    Browse and copy slugs from https://openrouter.ai/models.
    """
    return LLM(
        model=f"openrouter/{os.getenv('OPENROUTER_MODEL', 'deepseek/deepseek-chat')}",
...
# cut off mid-sentence. OpenRouter has no such cap, but it is only as reliable
# as the upstream model it routes to, so it rides shotgun instead.
# Set LLM_PROVIDER=openrouter in .env to flip the order.
PRIMARY_PROVIDER = os.getenv("LLM_PROVIDER", "groq").lower()


def make_llm(groq_tokens, openrouter_tokens):
    ...
    stay under GROQ_TPM_LIMIT (prompt + output), OpenRouter can breathe.
    """
    has_groq = bool(os.getenv("GROQ_API_KEY"))
    has_openrouter = bool(os.getenv("OPENROUTER_API_KEY"))

    groq = make_groq_llm(groq_tokens) if has_groq else None
    openrouter = make_openrouter_llm(openrouter_tokens) if has_openrouter else None

    if PRIMARY_PROVIDER == "groq":
        primary, secondary = groq, openrouter
    else:
        primary, secondary = openrouter, groq

    primary = primary or secondary
    if primary is None:
        raise RuntimeError("Set GROQ_API_KEY and/or OPENROUTER_API_KEY in .env")
...
# Budgets: (groq, openrouter). The Groq numbers keep prompt + output under
# GROQ_TPM_LIMIT; the OpenRouter numbers are what these reports actually need.
...
def fetch_jira_ticket(bug_id):
    ...
    try:
        ...
        if os.path.exists(cached):
            ...
            # explicit encoding: Windows would otherwise read this as cp1252
