...
def test_env_backed_credentials_are_not_reported_as_secrets(playwright_bundle, test_cases):
    """The secure pattern must not warn.

    Measured on a real run: the substring matcher fired on
    "const PASSWORD = process.env.TEST_PASSWORD || 'password-placeholder';"
    because the lowercased text contains "password =". That is exactly the
    code the agent is supposed to write, and the only way to silence it was to
    stop naming the variable PASSWORD - which teaches readers that secret
    warnings are noise.
    """
    safe = playwright_bundle.model_copy(
        update={
            "files": [
                playwright_bundle.files[0].model_copy(
                    update={
                        "content": playwright_bundle.files[0].content
                        + "\nconst USERNAME = process.env.TEST_USERNAME || 'username-placeholder';"
                        + "\nconst PASSWORD = process.env.TEST_PASSWORD || 'password-placeholder';"
                        + "\nconst API_KEY = process.env.API_KEY || 'your-api-key-here';"
                        + "\nawait page.getByTestId('task-list').click();"
                    }
                )
            ]
        }
    )
    result = validate_playwright(safe, test_cases, "VWO-48")
    secret_warnings = [w for w in result.warnings if "hard-coded" in w]
    assert secret_warnings == [], secret_warnings


@pytest.mark.parametrize(
    "snippet, expected",
    [
        ("const PASSWORD = 'hunter2';", "hard-coded password"),
        ('const apiKey = "abcd1234efgh5678";', "hard-coded API key"),
        ("const k = 'sk-abcdefghijklmnopqrstuvwxyz';", "hard-coded secret key"),
        (
...
def test_real_secrets_are_still_caught(playwright_bundle, test_cases, snippet, expected):
    leaky = playwright_bundle.model_copy(
        update={
            "files": [
                playwright_bundle.files[0].model_copy(
                    update={"content": playwright_bundle.files[0].content + "\n" + snippet}
                )
            ]
        }
    )
    result = validate_playwright(leaky, test_cases, "VWO-48")
    assert any(expected in w for w in result.warnings), (snippet, result.warnings)
