...
def test_a_script_needing_configuration_does_not_lower_coverage(
    analysis, test_cases, playwright_bundle
):
    """NEEDS_CONFIGURATION is a readiness fact, not a coverage gap.

    Measured on a real run: 3 requirements, all traced, all automated, and
    requirement_coverage_pct came back 0.0% because the bundle listed missing
    selectors. Manual-only coverage scores COVERED, so automating a
    requirement used to *lower* its score - and since a ticket rarely carries
    real selectors, the headline number read 0% on almost every good run.
    """
    from jira_qa_crew.models import AutomationReadiness

    # Same bundle twice: everything intended for automation IS automated. The
    # only difference is whether the script still needs configuration.
    ready_bundle = playwright_bundle.model_copy(
        update={
            "readiness": AutomationReadiness.READY,
            "missing_information": [],
        }
    )
    needs_config = playwright_bundle.model_copy(
        update={
            "readiness": AutomationReadiness.NEEDS_CONFIGURATION,
            "missing_information": ["Actual selectors for the cart total"],
        }
    )

    ready = build_coverage(analysis, test_cases, ready_bundle)
    coverage = build_coverage(analysis, test_cases, needs_config)

    # Coverage is unchanged by the script not being runnable...
    assert coverage.requirement_coverage_pct == ready.requirement_coverage_pct
    assert coverage.covered_requirements == ready.covered_requirements
    # ...but the fact is still reported, just on its own axis.
    assert coverage.automation_execution_ready is False
    assert coverage.automation_blockers == ["Actual selectors for the cart total"]
    assert ready.automation_execution_ready is True
