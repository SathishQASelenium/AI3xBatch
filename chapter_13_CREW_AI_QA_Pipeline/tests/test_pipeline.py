...
def test_a_too_thin_repair_is_not_told_to_avoid_new_content():
    """A "too thin" rejection must not carry the anti-fabrication clause.

    Measured against a live provider: told both to lengthen a section and not
    to invent content, the model left the section short and the single repair
    attempt failed identically to the first, burning two calls for nothing.
    """
    from crewai import Task

    task = Task.model_construct(description="original body")
    QAPipeline._append_repair_instruction(
        task, ["Section 9 (High-Level Test Scenarios) is empty or too thin"]
    )

    assert "Do not invent new content" not in task.description
    assert "Expand it with real substance" in task.description
    # Anti-hallucination must survive the rewording, just not as a gag order.
    assert "invent product behaviour" in task.description


def test_a_fabrication_repair_keeps_the_anti_invention_clause():
    from crewai import Task

    task = Task.model_construct(description="original body")
    QAPipeline._append_repair_instruction(
        task, ["Scenario SC-1 references unknown ids: REQ-999"]
    )

    assert "Do not invent new content" in task.description
    assert "Expand it with real substance" not in task.description
