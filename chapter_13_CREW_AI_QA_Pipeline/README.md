...
| --- | --- |
| `ruff check .` | clean |
| `pytest` | 268 passed, 3 skipped (the 3 are opt-in live tests) |
...
tests/                          268 tests, no live Jira or LLM
...
Requires Python **3.11 or 3.13**. Not 3.14 - see below.
...
`crewai` declares `Requires-Python >=3.10,<3.14`, and pip enforces that before
it looks at versions, so an exact pin does not help:

```text
ERROR: Could not find a version that satisfies the requirement crewai==1.15.17
```

On a machine where 3.14 is the only interpreter, `requirements-py314.txt` has a
verified workaround and the full procedure. It needs `uv`, not pip, and it is
four steps rather than one, because resolving for 3.13 downloads cp313 binary
wheels that cannot load on 3.14.

That file is a repaired install, not a clean resolve. It drifts from
`requirements.txt`, it is verified only on Windows + CPython 3.14.4, and
nothing upstream promises 3.14 support, so any upgrade can silently reintroduce
a package that breaks. Prefer installing 3.13.

The Dockerfile (`python:3.13-slim`) and CI (3.11, 3.13) use `requirements.txt`
and are unaffected.
...
Without activating it, call the venv's own executable. On Windows a bare
`streamlit` usually resolves to a different interpreter that has no `crewai`
installed, so prefer this form:
...
Expect roughly 3-6 minutes per ticket: four sequential LLM stages, plus any
retry. Artifacts are written to `outputs/RUN-<timestamp>/<TICKET>/` and offered
as downloads.

**Restart the server after editing anything in `prompts/`.** The YAML loader is
`lru_cache`d (`crew/prompts.py`), so Streamlit's auto-rerun keeps serving the
prompt already in memory. Stop the process and start it again.
