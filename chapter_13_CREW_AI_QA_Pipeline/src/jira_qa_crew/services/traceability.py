...
def build_coverage(
    ...
    return CoverageReport(
        ...
        automation_execution_ready=not (bundle.missing_information if bundle else []),
        automation_blockers=list(bundle.missing_information) if bundle else [],
    )
...
def _status_for(
    ...
    if bundle and bundle.missing_information:
        # Covered, not partial. Every test case that was meant to be automated
        # exists; the script needing selectors is a *readiness* fact, reported
        # separately as CoverageReport.automation_execution_ready.
        #
        # This used to return PARTIAL, which made the headline coverage number
        # perverse: a requirement covered by manual cases alone scores COVERED
        # (see the branch above), so automating it *lowered* the score. Since a
        # ticket rarely carries real selectors, NEEDS_CONFIGURATION is the
        # normal outcome, and requirement_coverage_pct read 0% on nearly every
        # successful run.
        return (
            CoverageStatus.COVERED,
            "Covered by automated tests; the script still needs configuration: "
