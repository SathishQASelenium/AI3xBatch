...
class QAPipeline:
    ...
    def _run_ticket(self, result: TicketResult, mode: IntegrationMode | None) -> None:
        ...
        # Kept as soon as it is validated, not at the end of the pipeline: a
        # later stage raises, and anything still held in a local is lost along
        # with the artifacts and the manifest counts that read from it.
    ...
    def _append_repair_instruction(task: Task, problems: list[str]) -> None:
        ...
        The closing guidance is chosen to match the problems. A blanket "do not
        invent content" is right for a fabrication error but actively wrong for
        a too-thin one: told to lengthen and not to lengthen at once, a model
        stays short and the retry fails identically to the first attempt.
        """
        marker = "\n\n### CORRECTION REQUIRED (single retry)\n"
        base = task.description.split(marker)[0]
        bullets = "\n".join(f"- {p}" for p in problems[:10])
        thin = any(
            "too thin" in p or "is empty" in p or "at least" in p for p in problems
        )
        if thin:
            guidance = (
                "At least one problem above is a section or field that is too "
                "short. Expand it with real substance drawn from the analysis "
                "and the rest of this plan - describe scope, grouping, "
                "priority and rationale. A cross-reference such as 'see "
                "above' is not content. Do NOT invent product behaviour, "
                "environments or ids that the ticket does not support; write "
                "about what is known, including what is still unknown and why "
                "it matters."
            )
        else:
            guidance = (
                "Do not invent new content to satisfy a check: if information "
                "is genuinely missing, say so in the relevant field rather "
                "than fabricating it."
            )
        task.description = (
            f"{base}{marker}"
