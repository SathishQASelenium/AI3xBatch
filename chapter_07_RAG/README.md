...
- **Source doc:** a sample VWO PRD (`Basic_RAG/data/*.pdf`) — drop in any PDF to try your own.
...
- `Basic_RAG/data/` — source PDF(s) to ingest.
- `Basic_RAG/rag-explorer/` — the React + Express app:
  ...
  - `src/` — the pipeline visualisation UI (Vite + React).
...
npm run dev                # starts ChromaDB (:8000) + Express API (:8787) + Vite UI (:5175)
```

Open the printed Vite URL, click **Ingest PDF**, then ask a question. See
